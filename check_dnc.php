...
if (isset($input['email'])) {
    ...
    $email = filter_var($email, FILTER_SANITIZE_EMAIL);
        ...
        $check_dnc = check_dnc($db, $email, $phone_number);
        if($check_dnc > 0){
            echo json_encode([
                'status' => true,
...
function check_dnc($db, $email, $phone_number) {
    $stmt = $db->prepare("SELECT COUNT(*) FROM leads WHERE (email = :email OR phone_number = :phone_number) AND status = 'DNC Registry'");
