...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $mobile_number = '';
    $email = '';
    $source_url = 'https://janicez539.sg-host.com/';
    $webhook_data = array(
        'source_url' => "https://janicez539.sg-host.com/",
        'client_id' => null,
        'project_id' => null,
    ...
    $data .= "- Source Url: https://janicez539.sg-host.com/ " . PHP_EOL;
    ...
    foreach ($_POST as $key => $value) {
        ...
        if($key == 'When_you\'re_collecting_your_keys?') {
            $additional_data[] = array(
                "key" => "When you are collecting you keys?",
                "value" => $value
            );
            $data .= '- When you are collecting you keys?: '. $value . PHP_EOL;
        }
        if($key == 'HBD_Flat_Type') {
            $additional_data[] = array(
                "key" => str_replace("_"," ",$key),
                "value" => $value
            );
    ...
    if($is_discord == 1){
       $a = sendDiscordMsg($data); print_r($a ); die();
    }
...
{ 
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://discord.com/api/webhooks/1293794049177948170/FvS1yTbhT1zou0XVOWM6vMC3-Of10XEprGCBuY6ktEMLONIW_gXlAO-RgKfUCI-ZN1cH',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
