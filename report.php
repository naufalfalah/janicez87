...
$sql = "SELECT domain, COUNT(*) AS total_leads FROM (
    SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(source_url, '/', 3), '/', -1) AS domain, source_url, COUNT(*) AS total_leads 
    FROM leads";
...
$sql .= " AND status = 'clear'  
          AND leads.email NOT LIKE '%jome%' 
          AND leads.email NOT LIKE '%test%' 
          GROUP BY email,phone_number 
        ) AS TEMP GROUP BY domain;";
...
<?php if (!isset($_GET['datefilter']) && empty($_GET['datefilter'])) { ?>
