...
function saveLeadData($db, $data) { 
    ...
    $stmt = $db->prepare("INSERT INTO leads (source_url, client_id, project_id, ip_address, name, email, phone_number, status, is_send_discord, is_verified) VALUES (:source_url, :client_id, :project_id, :ip_address, :firstname, :email, :ph_number, :status, :is_send_discord, :is_verified)");
...
//     try {
//         $commonFields = [
//             "source_url", "client_id", "project_id", "ip_address", 
//             "firstname", "email", "ph_number", "status", "is_send_discord", "is_verified"
//         ];

//         // 🛑 Check which keys exist in data
//         $leadsData = array_intersect_key($data, array_flip($commonFields));

//         // 🛑 Debugging: Print the final array before execution
//         if (count($leadsData) !== count($commonFields)) {
//             return json_encode([
//                 "status" => "error",
//                 "message" => "❌ Mismatched Fields",
//                 "expected_keys" => $commonFields,
//                 "received_keys" => array_keys($leadsData)
//             ]);
//         }

//         // ✅ Insert Query (MAKE SURE PLACEHOLDERS MATCH DATA KEYS)
//         $stmt = $db->prepare("
//             INSERT INTO leads 
//             (source_url, client_id, project_id, ip_address, name, email, phone_number, status, is_send_discord, is_verified) 
//             VALUES 
//             (:source_url, :client_id, :project_id, :ip_address, :firstname, :email, :ph_number, :status, :is_send_discord, :is_verified)
//         ");

//         // ✅ Execute Query
//         if (!$stmt->execute($leadsData)) {
//             return json_encode([
//                 "status" => "error",
//                 "message" => "❌ Lead Insertion Failed",
...
//             return json_encode([
//                 "status" => "error",
//                 "message" => "❌ No Lead ID Generated!"
//             ]);
//         }

//         // ✅ Handle Extra Fields (Lead Details)
//         $otherFields = array_diff_key($data, array_flip($commonFields));
//         if (!empty($otherFields)) {
//             $stmt = $db->prepare("INSERT INTO lead_details (lead_id, lead_form_key, lead_form_value) VALUES (:lead_id, :lead_form_key, :lead_form_value)");
//             foreach ($otherFields as $key => $value) {
...
//                     return json_encode([
//                         "status" => "error",
//                         "message" => "❌ Lead Detail Insert Failed for $key",
//                         "errorInfo" => $stmt->errorInfo()
//                     ]);
//                 }
//             }
...
//         return json_encode([
//             "status" => "error",
//             "message" => "❌ Exception in saveLeadData",
