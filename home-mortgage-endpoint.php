...
function saveLeadData($db, $data) {
    $commonFields = ["source_url","name", "mobile_number", "status", "is_send_discord","is_verified"];
    $leadsData = array_intersect_key($data, array_flip($commonFields));

    print_r($leadsData);
    $stmt = $db->prepare("INSERT INTO leads (source_url, name, phone_number, status, is_send_discord, is_verified) VALUES (:source_url, :name, :mobile_number, :status, :is_send_discord, :is_verified)");
    $stmt->execute($leadsData);
   echo $leadId = $db->lastInsertId();
